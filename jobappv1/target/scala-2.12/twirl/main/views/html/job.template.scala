...
<div class="topnav">
  ...
  <body>
    """),_display_(/*71.6*/helper/*71.12*/.form(routes.JobController.updateJob(index))/*71.56*/ {_display_(Seq[Any](format.raw/*71.58*/("""
        ...
        """),_display_(/*74.10*/helper/*74.16*/.inputText(form(job.getTitle()))),format.raw/*74.48*/("""

        """),_display_(/*76.10*/helper/*76.16*/.textarea(form(job.getDescription()))),format.raw/*76.53*/("""

        """),_display_(/*78.10*/helper/*78.16*/.inputText(form(job.getSalary().toString()))),format.raw/*78.60*/("""

        """),format.raw/*80.9*/("""<button type="submit">Update Job</button>
    """)))}),format.raw/*81.6*/("""

  """),format.raw/*83.3*/("""</body>
<html>
"""))
      }
    }
